	...
	struct mmc_card *oldcard)
{
	 ...
	 */
	if ((card->ext_csd.hs_max_dtr != 0) || (card->ext_csd.hs200_max_dtr != 0)) {
		err = 0;
	...
	if (mmc_card_highspeed(card) || mmc_card_hs200(card)) {
		if(mmc_card_hs200(card) && (max_dtr > card->ext_csd.hs200_max_dtr))
			max_dtr = card->ext_csd.hs200_max_dtr;
		else if (mmc_card_highspeed(card) && (max_dtr > card->ext_csd.hs_max_dtr)){
	...
	if (mmc_card_hs200(card)) {
		...
		/**
		 * switch to hs400 if device and host all can support
		 */
