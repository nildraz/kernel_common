...
static int dw_i2c_probe(struct platform_device *pdev)
{
	...
	dev->master_cfg =  DW_IC_CON_MASTER | DW_IC_CON_SLAVE_DISABLE |
		DW_IC_CON_RESTART_EN | (val ? DW_IC_CON_SPEED_FAST : DW_IC_CON_SPEED_STD);
...
//	}
	...
	r = devm_request_irq(&pdev->dev, dev->irq, i2c_dw_isr, IRQF_SHARED|IRQF_NO_SUSPEND,
			pdev->name, dev);
	if (r) {
...
int dw_i2c_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct dw_i2c_dev *i_dev = platform_get_drvdata(pdev);
...
{
	struct dw_i2c_dev *i2c_dev = dev_get_drvdata(dev);
...
int dw_i2c_runtime_resume(struct device *dev)
{
	struct dw_i2c_dev *i2c_dev = dev_get_drvdata(dev);
	if (!IS_ERR(i2c_dev->clk))
...
EXPORT_SYMBOL(dw_i2c_runtime_resume);
...
#endif
struct dev_pm_ops dw_i2c_dev_pm_ops = {
	.suspend_noirq = dw_i2c_suspend,
	.resume_noirq = dw_i2c_resume,
