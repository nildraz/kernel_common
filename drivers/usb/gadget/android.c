...
#define NUFRONT_7789_ADB
#ifdef NUFRONT_7789_ADB
#include "f_adb.c"
#endif
#include "f_fs.c"
/* should define AUDIO */
...
static void android_work(struct work_struct *data)
{
	...
	if (cdev->config)
		...
		if (dev->connected) {
			printk(KERN_INFO "usb connect, wake_lock\n");
			wake_lock(&g_wake_lock);
		} else {
			printk(KERN_INFO "usb disconnect, wake_unlock\n");
