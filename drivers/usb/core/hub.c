			...
			struct usb_device *udev, unsigned int delay, bool warm)
{
	...
	for (delay_time = 0;
		...
		if (!(portstatus & USB_PORT_STAT_RESET)) {
			dev_dbg (hub->intfdev, "finish reset with portstatus=0x%x\n", portstatus);
			break;
		}
			...
			struct usb_device *udev, unsigned int delay, bool warm)
{
	...
	dev_dbg(hub->intfdev, "reset done with status = %d\n", status);
	return status;
}
...
static void hub_events(void)
{
	...
	while (1) {
		...
		for (i = 1; i <= hub->descriptor->bNbrPorts; i++) {
			...
			if (ret < 0)
				continue;
			dev_dbg (hub_dev, "portstatus=0x%x, portchange=0x%x\n", portstatus, portchange);
...
/*+++from William*/
			if ((portstatus & USB_PORT_STAT_LINK_STATE) ==
				USB_SS_PORT_LS_COMP_MOD)
			{
                printk("ignore port change events on port of compliance mode\n");
				dev_dbg(hub_dev, "ignore port change events on "
						"port of compliance mode\n");
				connect_change = 0;
			}
/*---from William*/
			if (connect_change)
