...
#define kPosReverse  0  
#define kPosReverseY 0  
#define kPosReverseX 0 

/* Panel size is x=800 y = 480 need to divide */
//#define KPanelGetMax_X  1024
//#define KPanelGetMax_Y  600
#define KPanelGetMax_X  800
#define KPanelGetMax_Y  480
...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
    ...
    #ifdef CONFIG_DEBUG_GT9XX
    printk("[Gt9xx]gtp_touch_down start.x=%d y=%d w=%d\n" , x, y, w);
    #endif
    ...
    if(KPanelGetMax_X!=GTP_MAX_WIDTH)
        x = (x*GTP_MAX_WIDTH)/KPanelGetMax_X;
    if(KPanelGetMax_Y!=GTP_MAX_HEIGHT)
        y = (y*GTP_MAX_HEIGHT)/KPanelGetMax_Y;
#else
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    while(retry++ < 10)
    {
        ...
        msleep(5);
    #endif
        gtp_reset_guitar(ts->client, 20); //jackson, to reset driver firstly when wakeup from S2 power saving
                                          //for solving i2c NO ACK
 ...
 */
    ...
    #ifdef CONFIG_DEBUG_GT9XX
    printk("[Gt9xx]gtp_init_panel 1.Config Groups\' Lengths: %d, %d, %d, %d, %d, %d", 
...
#else
    GTP_DEBUG("gtp_fb_notifier_callback event=%d FB_EVENT_RESUME=%d FB_EVENT_SUSPEND=%d\n" , event , FB_EVENT_RESUME , FB_EVENT_SUSPEND);
	if ( ts) {
