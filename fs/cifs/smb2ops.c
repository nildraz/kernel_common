	      ...
	      struct smb_rqst *rqst, int enc)
{
    ...
    cifs_dbg(VFS, "%s:%d: tr_hdr->OriginalMessageSize=0x%x\n", __func__, __LINE__,
			 tr_hdr->OriginalMessageSize);
    ...
    cifs_dbg(VFS, "%s:%d: tr_hdr->OriginalMessageSize=0x%x enc=%d\n", __func__, __LINE__,
			 tr_hdr->OriginalMessageSize, enc);
    ...
    asg = kmalloc(sizeof(struct scatterlist), GFP_KERNEL);
    assoc_data = rqst[0].rq_iov[0].iov_base + 20;
    assoc_len = rqst[0].rq_iov[0].iov_len - 20;

    sg_init_one(asg, assoc_data, assoc_len);
    smb2_sg_set_buf(asg, assoc_data, assoc_len);
    aead_request_set_assoc(req, asg, assoc_len);

	aead_request_set_callback(req, CRYPTO_TFM_REQ_MAY_BACKLOG,
				  crypto_req_done, &tresult);

	rc = enc ? crypto_aead_encrypt(req)
                                : crypto_aead_decrypt(req);

	if (!rc && enc)
		memcpy(&tr_hdr->Signature, sign, SMB2_SIGNATURE_SIZE);
    cifs_dbg(VFS, "%s:%d: tr_hdr->Signature=%02X %02X %02X %02X\n", __func__, __LINE__,
			 tr_hdr->Signature[0]&0xff, tr_hdr->Signature[1]&0xff, tr_hdr->Signature[2]&0xff,
			 tr_hdr->Signature[3]&0xff);
		       ...
		       struct smb_rqst *new_rq, struct smb_rqst *old_rq)
{
	...
	cifs_dbg(FYI, "(JJJ)num_rqst %d\n", num_rqst);
	for (i = 1; i < num_rqst; i++) {
		npages = old_rq[i - 1].rq_npages;
        cifs_dbg(FYI, "(JJJ)[%d]npages %d\n", i,npages);
		 ...
		 unsigned int npages, unsigned int page_data_size)
{
	...
	length = rdata->copy_into_pages(server, rdata, bvec, npages, data_len);
