		...
		struct smb_rqst *rqst)
{
	...
	if (server->vals->header_preamble_size == 0) {
		...
		rc = smb_send_kvec (server, &hiov, 1, &sent);
		if (rc < 0)
			goto uncork;
	...
	for (j = 0; j < num_rqst; j++) {
		...
		for (i = 0; i < n_vec; i++) {
			...
			size += iov[i].iov_len;
			cifs_dbg(FYI, "(JJJ)iov[%d].iov_len=%u\n", i, iov[i].iov_len);

		}

		rc = smb_send_kvec (server, iov, n_vec, &sent);
		cifs_dbg(FYI, "(JJJ)sent=%u rc=%d\n", sent, rc);
		if (rc < 0)
			goto uncork;
		...
		total_len += sent;
                ...
                for (i = 0; i < rqst[j].rq_npages; i++) {
                    ...
                    cifs_rqst_page_to_kvec(&rqst[j], i, &p_iov);
                    rc = smb_send_kvec(server, &p_iov, 1, &sent);
                    cifs_dbg(FYI, "(JJJ)sent=%u rc=%d, p_iov.iov_len=%u\n", sent, rc, p_iov.iov_len);
                    kunmap(rqst[j].rq_pages[i]);
                    if (rc < 0)
                        break;
