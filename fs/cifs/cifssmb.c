...
decode_lanman_negprot_rsp(struct TCP_Server_Info *server, NEGOTIATE_RSP *pSMBr)
{
	...
	if (tmp == -1) {
		...
		ts = cnvrtDosUnixTm(rsp->SrvTime.Date,
				    rsp->SrvTime.Time, 0);
		cifs_dbg(FYI, "SrvTime %ld sec since 1970 (utc: %ld) diff: %ld\n",
			 ts.tv_sec, utc.tv_sec,
			 utc.tv_sec - ts.tv_sec);
...
cifs_readv_receive(struct TCP_Server_Info *server, struct mid_q_entry *mid)
{
	...
	length = rdata->read_into_pages(server, rdata, NULL, 0, data_len);
	if (length < 0)
		return length;
			       ...
			       const int acl_type, const int size_of_data_area)
{
	...
	posix_acl_xattr_header *local_acl = (void *)trgt;
...
/* skip past access ACEs to get to default ACEs */
	...
	if ((buflen == 0) || (local_acl == NULL)) {
		...
		posix_acl_xattr_entry *ace = (void *)(local_acl + 1);
			       ...
			       const int buflen, const int acl_type)
{
	...
	posix_acl_xattr_header *local_acl = (void *)pACL;
	posix_acl_xattr_entry *ace = (void *)(local_acl + 1);
	int count;
	int i;
