...
static const struct file_operations smbSC_proc_fops;
static const struct file_operations otherRC_proc_fops;
...
cifs_proc_init(void)
{
    ...
    proc_create("smbSC", 0, proc_fs_cifs, &smbSC_proc_fops); //JJJ save error code
    proc_create("otherRC", 0, proc_fs_cifs, &otherRC_proc_fops); //JJJ save error code
#ifdef CONFIG_CIFS_SMB_DIRECT
...
cifs_proc_clean(void)
{
	...
	remove_proc_entry("smbSC", proc_fs_cifs); // JJJ save error
	remove_proc_entry("otherRC", proc_fs_cifs); // JJJ save error
#ifdef CONFIG_CIFS_SMB_DIRECT
...
// JJJ save error
static int smbSC_proc_show(struct seq_file *m, void *v)
{
	seq_printf(m, "0x%x\n", smbSC);
	return 0;
}

static int smbSC_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, smbSC_proc_show, NULL);
}

 static const struct file_operations smbSC_proc_fops = {
	.open		= smbSC_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
// JJJ save error
static int otherRC_proc_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", otherRC);
	return 0;
}

static int otherRC_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, otherRC_proc_show, NULL);
}

 static const struct file_operations otherRC_proc_fops = {
	.open		= otherRC_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
