...
#include "smb2pdu.h"
// JJJ save error
unsigned int smbSC=0;   /* to save smb/smb2 status code*/
int otherRC=0;   /* To save other result code, such as request_key */
...
*/
static void seq_show_option(struct seq_file *m, const char *name,
                                   const char *value)
{
        seq_putc(m, ',');
        seq_escape(m, name, ",= \t\n\\");
	      ...
	      int flags, const char *dev_name, void *data, bool is_smb3)
{
	...
	cifs_dbg(FYI, "smbSC=0\n");
    smbSC = 0;// JJJ save error
    otherRC = 0;
...
{
    struct dentry *root;  //JJJ save error code
    root = cifs_smb3_do_mount(fs_type, flags, dev_name, data, true);
	if (IS_ERR(root))
    {
        cifs_dbg(FYI, "Save error %d\n", (int)root);
        otherRC = (long)root;
    }
    return root;
...
{
    struct dentry *root;  //JJJ save error code
    root = cifs_smb3_do_mount(fs_type, flags, dev_name, data, false);
	if (IS_ERR(root))
    {
        cifs_dbg(FYI, "Save error %d\n", (int)root);
        otherRC = (long)root;
...
static int
cifs_setlease(struct file *file, long arg, struct file_lock **lease)
{
	/*
