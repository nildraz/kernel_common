...
/* ====To save error====================
   ...
   10 - Warning
   11 - Error                            */
void backup_smbX_status_code( unsigned int smbStatusCode)
{
    unsigned int oldL, newL;

    cifs_dbg(FYI, "backup_smbX_status_code=0x%x\n", smbStatusCode);
    /* smb1 and share folder name is wrong, NT_STATUS_MORE_PROCESSING_REQUIRED happen many times */
    if( (NT_STATUS_MORE_PROCESSING_REQUIRED) == smbStatusCode ||
         (NT_STATUS_FS_DRIVER_REQUIRED)==smbStatusCode)
        return;
    oldL = (smbSC & 0xC0000000) >> 30;
    newL = (smbStatusCode & 0xC0000000) >> 30;
    cifs_dbg(FYI, "oldL=0x%x newL=0x%x\n", oldL, newL);
    if(smbSC==0 || (newL >0 && newL > oldL) )
        smbSC = smbStatusCode;

    cifs_dbg(FYI, "smbSC=0x%x\n", smbSC);
    return;
}
...
map_smb_to_linux_error(char *buf, bool logErr)
{
    ...
    backup_smbX_status_code( le32_to_cpu(smb->Status.CifsError) ); // JJJ save error code
